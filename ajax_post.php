...
if($_POST) {
    ...
    if(isset ($_POST['source_id'])){ $source_id = $_POST['source_id'];}else{$source_id='Custom_Form_Widget';}
    ...
    //send email
    if(isset ($_POST['emailto'])){ $to = $_POST['emailto'];}else{$to='info@forexblast.com';}
    if(isset ($_POST['emailsubject'])){ $subject = $_POST['emailsubject'];}else{$subject='WWM Registration';}
    ...
    if($output->isSuccessful==true){
        ...
        if($platformID=='2'){
            echo json_encode(array('acc' => $accountInfo,'password'=>$_POST['pwd']));
        }elseif($platformID=='1'){
            echo json_encode(array('acc' => $u_name,'password'=>$_POST['pwd']));
        }
    }
